...
// Obtenir la liste des emplois du temps pour les cours de l'enseignant
$stmt = $pdo->prepare(" 
    SELECT 
...
foreach ($emplois_du_temps as $cours) {
    if (!isset($couleurs[$cours['module_nom']])) {
        // Générer une couleur claire aléatoire
        $couleur = sprintf('#%02X%02X%02X', mt_rand(200, 255), mt_rand(200, 255), mt_rand(200, 255));
        $couleurs[$cours['module_nom']] = $couleur;
    }
}
    ...
    <div class="container">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h2 class="mb-0"><i class="fas fa-calendar-alt me-2"></i>Emploi du temps - Enseignant</h2>
                                    ...
                                    <?php foreach ($heures as $heure): ?>
                                        <?php
                                            ...
                                            foreach ($emplois_du_temps as $cours) {
                                                ...
                                                // Ajuster la comparaison avec l'heure
                                                if ($jour_cours === $jour_en && $heure == $heure_format) {
                                            ...
                                            if (!$cours_affiche) {
                                                // Afficher une cellule vide ou une pause
                                                if ($heure == '12:00-14:00') { // Pause pour cette plage horaire
                                                    echo '<td class="cell-pause"></td>';
                                                } else {
