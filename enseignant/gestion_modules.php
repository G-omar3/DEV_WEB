...
$stmt = $pdo->prepare("SELECT c.id AS classe_id, c.nom AS classe_nom, u.name AS professeur_nom, u.prenom AS professeur_prenom 
                       FROM Classe c 
                       JOIN users u ON c.professeur_responsable_id = u.id 
...
$stmt = $pdo->prepare("SELECT m.id, m.nom, m.volume_horaire, u.name AS enseignant_nom, u.prenom AS enseignant_prenom 
                       FROM Module m 
...
// Obtenir uniquement le professeur connecté
$stmt = $pdo->prepare("SELECT id, `name`, prenom FROM users WHERE role = 'teacher' AND id = ?");
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="enseignant_id" name="enseignant_id" required>
                                    <?php foreach ($enseignants_disponibles as $enseignant): ?>
                                        <option value="<?= $enseignant['id'] ?>"><?= htmlspecialchars($enseignant['name'] . ' ' . $enseignant['prenom']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
