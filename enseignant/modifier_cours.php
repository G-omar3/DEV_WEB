...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['modifier'])) {
    ...
    $stmt = $pdo->prepare("SELECT id FROM users WHERE role = 'student' AND id IN (SELECT etudiant_id FROM Inscription WHERE classe_id = ?)");
                ...
                <div class="form-group col-md-4">
                    ...
                    <select id="enseignant_id" name="enseignant_id" class="form-control" required>
                        <?php foreach ($enseignants as $enseignant): ?>
                            <option value="<?= $enseignant['id'] ?>" <?= ($enseignant['id'] == $cours['enseignant_id']) ? 'selected' : '' ?>><?= htmlspecialchars($enseignant['name'] . ' ' . $enseignant['prenom']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group col-md-4">
