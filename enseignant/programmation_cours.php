...
$enseignants_stmt = $pdo->query("SELECT id, `name`, prenom FROM users WHERE role = 'teacher'");
...
// Récupérer les étudiants inscrits à la classe via la table 'inscription'
$students_stmt = $pdo->prepare("SELECT etudiant_id FROM inscription WHERE classe_id = ?");
$students_stmt->execute([$classe_id]);
$students = $students_stmt->fetchAll();

// Ajouter une notification pour chaque étudiant
$notification_message = "Le cours a été programmé. Veuillez vérifier l'emploi du temps.";
...
if (isset($_GET['action']) && $_GET['action'] == 'supprimer' && isset($_GET['id'])) {
    ...
    // Ajouter une notification pour chaque étudiant
    $notification_message = "Le cours a été supprimé. Veuillez consulter l'emploi du temps mis à jour.";
    foreach ($students as $student) {
        $insertNotificationStmt = $pdo->prepare("INSERT INTO Notification (utilisateur_id, message, vu, date_heure) VALUES (?, ?, 0, NOW())");
        $insertNotificationStmt->execute([$student['etudiant_id'], $notification_message]);
    }
...
$stmt = $pdo->prepare("
    SELECT c.id, m.nom AS module, u.name AS enseignant_nom, u.prenom AS enseignant_prenom, s.nom AS salle, ed.date_heure, ed.heure_debut, ed.heure_fin
...
if (isset($_GET['enseignant_id'])) {
    ...
    $stmt = $pdo->prepare("
        SELECT d.jour, d.heure_debut, d.heure_fin, u.name, u.prenom
        FROM Disponibilite d
        JOIN users u ON d.enseignant_id = u.id
        ...
        <div class="row">
                            ...
                            <div class="mb-3">
                                ...
                                <select id="enseignant_id" name="enseignant_id" class="form-select" required onchange="loadDisponibilites(this.value)">
                                    ...
                                    <?php foreach ($enseignants as $enseignant): ?>
                                        <option value="<?= $enseignant['id'] ?>">Mr/Mme <?= htmlspecialchars($enseignant['name'] . ' ' . $enseignant['prenom']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
            ...
            <div class="col-lg-8">
                ...
                <h3 class="mb-0"><i class="fas fa-calendar-alt me-2"></i>Cours Programmés</h3>
                        <a href="dashboardenseignant.php" class="btn btn-custom me-3">
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach(disponibilite => {
                            disponibilitesHTML += `
                                <p>
                                    <strong>${disponibilite.jour}</strong> : Mr/Mme ${disponibilite.name} ${disponibilite.prenom}
