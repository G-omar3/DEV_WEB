...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ajouter_gestionnaire'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO users (`name`, prenom, email, `password`, `role`) VALUES (?, ?, ?, ?, 'Gestionnaire')");
...
$stmt = $pdo->query("SELECT * FROM users WHERE role = 'Gestionnaire'");
                                ...
                                <tbody>
                                    <?php if (count($gestionnaires) > 0): ?>
                                        <?php foreach ($gestionnaires as $gestionnaire): ?>
                                            <tr>
                                                <td><?= htmlspecialchars($gestionnaire['name']) ?></td>
