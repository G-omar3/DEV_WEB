...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['designer_responsable'])) {
    ...
    if (!$roleExists) {
        $stmt = $pdo->prepare("INSERT INTO users (id, role) VALUES (?, 'teacher')");
        $stmt->execute([$enseignant_id]);
    }
...
if (isset($_GET['action']) && $_GET['action'] == 'supprimer' && isset($_GET['classe_id'])) {
    ...
    if ($count == 0) {
        $stmt = $pdo->prepare("DELETE FROM users WHERE id = (SELECT professeur_responsable_id FROM Classe WHERE id = ?) AND role = 'student'");
        $stmt->execute([$classe_id]);
    }
...
$stmt = $pdo->query("SELECT id, `name`, prenom FROM users WHERE role = 'teacher'");
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <?php foreach ($classes as $classe): ?>
                                        <?php
                                        $stmt = $pdo->prepare("SELECT u.name, u.prenom FROM users u JOIN Classe c ON u.id = c.professeur_responsable_id WHERE c.id = ?");
                                        $stmt->execute([$classe['id']]);
                                        $responsable = $stmt->fetch();

                                        if ($responsable) {
                                            $responsable_nom = $responsable['name'];
